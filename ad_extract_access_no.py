...
class Extract(object):
    ...
    def update_company_plus_doc_meta_data_changed(self):
        ...
        address_info_lst = []
        sn = 1
        for company_id, info_tup in company_dct.items()[4:]:
            ...
            try:
                ...
                #address_info_lst.extend([business_address_tup, mailing_address_tup])
                print in_t_dct, sec_t_dct
                self.insert_business_mailing_address(m_conn, m_cur, [business_address_tup, mailing_address_tup])
                com_tup = (it_id, sec_id, company_id)
                print com_tup, '\n'
                if industry_type or sector:
                ...
                for url_tup in complete_urls[:]:
                    ...
                    period_of_report = doc_data_info_dct.get('period of report', '')
                    filling_date  = doc_data_info_dct.get('filing date', '')
                    dc_tup = (company_id, 'Periodic Financial Statement', filling_date, period_of_report, ac_n, sec_filing_number)
                    doc_lst.append(dc_tup)
                    print dc_tup
                if doc_lst:
                    insert_stmt = """ INSERT INTO document_master(company_id, document_type, document_release_date, document_to, assension_number, sec_filing_number) VALUES(%s, %s, %s, %s, %s, %s) """      
                    m_cur.executemany(insert_stmt, doc_lst)
                    m_conn.commit()
            except:
                f.write('-'.join((str(company_id), str(cdn)))+'\n')
        f.close()
        m_conn.close()
        return 
                ...
                if identInfo:
                    ...
                    if sector:
                        ...
                        if not sec_id:
                            ...
                            sec_t_dct[sec_lower] = sec_id
                        print sector
                        com_tup = (sec_id, company_id)
                        c_update_stmt = """ UPDATE company_mgmt SET sec_sector='%s' WHERE row_id='%s' """%com_tup
                        print c_update_stmt
                        #m_cur.execute(c_update_stmt)
                        m_conn.commit()
            #except:
