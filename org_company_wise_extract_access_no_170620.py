...
class Extract(object):
    ...
    def read_company_info_comapny_mgmt(self, i_cids=[]):
        ...
        if i_cids:
            ...
            read_qry = """  SELECT row_id, ticker, sec_cik, company_display_name FROM company_mgmt WHERE row_id in (%s);"""%(', '.join(map(lambda x:str(x), i_cids)))
        else:
        ...
        for company_id, info_tup in company_dct.items()[:]:
            ...
            if 1:
                ...
                for form_type in ['10-K', '10-Q']:
                    c_urls, comp_url, address_info = self.extract_info_fnc(tkr, form_type)
