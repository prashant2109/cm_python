...
class Extract(object):
    ...
    def read_company_info_comapny_mgmt(self):
        ...
        #read_qry = """  SELECT row_id, ticker, sec_cik, company_display_name FROM company_mgmt;"""
    ...
    def update_company_plus_doc_meta_data_changed(self):
        ...
        for company_id, info_tup in company_dct.items()[:]:
            ...
            try:
                ...
                '''
                com_tup = (uc_name, company_id)
                print com_tup, '\n'
                if uc_name:
                    #c_update_stmt = """ UPDATE company_mgmt SET sec_industry='%s', sec_sector='%s' WHERE row_id='%s' """%com_tup
                    c_update_stmt = """ UPDATE company_mgmt SET sec_name='%s' WHERE row_id='%s' """%com_tup
                    print c_update_stmt
                ...
                for url_tup in complete_urls[:]:
                    ...
                    dc_tup = (company_id, 'Periodic Financial Statement', filling_date, period_of_report, ac_n, sec_filing_number, docinfo_url, fm_type)
                    doc_lst.append(dc_tup)
                    print dc_tup
                    #sys.exit()
                #print doc_lst
                if doc_lst:
                    insert_stmt = """ INSERT INTO document_master(company_id, document_type, document_release_date, document_to, assension_number, sec_filing_number, url_info, filing_type) VALUES(%s, %s, %s, %s, %s, %s, %s, %s) """      
